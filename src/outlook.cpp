...
void DumpBuffer(FILE* out, const void* buffer, size_t len, int maxBinaryDumpLength);
...
void DumpRTF(FILE* out, const void* buffer, size_t len, int maxDumpLength)
{
	...
	{
		DumpBuffer(out, buffer, len, maxBinaryDumpLength);
		return;
	}
	...
	if (retlen > 0)
	{
		...
		// ZMM Fix this confusion
		_ASSERTE((maxDumpLength < 0) || (maxDumpLength == maxRtfDumpLength));
		bool singleLine = false;
		if (retlen <= maxRtfDumpLength)
			singleLine = true;
		else if (maxDumpLength > 0)
		{
			outlen = maxDumpLength;
			singleLine = true;
		}
		...
		// dst is oversized so inline is safe 
		dst[outlen] = 0;
		fprintf(out, "\"%s\"", dst);
...
void DumpBuffer(FILE* out, const void* buffer, size_t len, int maxDumpLength)
{
	...
	// ZMM Fix this confusion
	_ASSERTE((maxDumpLength < 0) || (maxDumpLength == maxBinaryDumpLength));
	bool singleLine = false;
	if (len <= maxBinaryDumpLength)
		singleLine = true;
	else if (maxDumpLength > 0)
	{
		outlen = maxDumpLength;
	...
	if (maxBinaryDumpLength == 0)
	{
		fprintf(out, "\n");
		return;
	}

	if (outlen > maxBinaryDumpLength)
		fprintf(out, "\n");
...
void DumpTextU16(FILE* out, const char* buffer, size_t len, int maxDumpLength)
{
	...
	// ZMM Fix this confusion
	_ASSERTE((maxDumpLength < 0) || (maxDumpLength == maxTextDumpLength));
	bool singleLine = false;
	if (len <= maxTextDumpLength)
		singleLine = true;
	else if (maxDumpLength > 0)
	{
		outlen = maxDumpLength;
	...
	if (singleLine)
	{
		wchar_t* buf = (wchar_t*)cfbf_malloc(len * 2 + 2);
	...
	else // if ((outlen > maxTextDumpLength))
	{
		fprintf(out, "\n");
	}

	std::string buff8 = UTF16ToUTF8(str, outlen);
	char* astr = (char*)buff8.c_str();
...
void DumpTextU8(FILE* out, const char* buffer, size_t len, int maxDumpLength)
{
	...
	if (singleLine)
	{
		char* buf = (char*)cfbf_malloc(outlen+1);
		for (int i = 0; i < (int)outlen; i++)  // Fix type casting ZMM
		{
			if ((str[i] == '\n') || (str[i] == '\r'))
				buf[i] = ' ';
			else
	...
	std::string txt(str, outlen);  // test
	...
	DirEntry* parent, unsigned long entry_id, int depth)
{
	...
	if (dumpMethod == 1)
	{
		DumpBuffer(out, data, data_len, maxBinaryDumpLength);
	}
	else if (dumpMethod == 2)
	{
		std::string data_utf8 = UTF16ToUTF8((wchar_t*)data, data_len / 2);  // just for testing

		int maxDumpLength = maxTextDumpLength;
		if (expandedPrint)
		{
			if (
				(propertyIdNumb == PidTagTransportMessageHeaders) ||
				(propertyIdNumb == PidTagBodyHtml) ||
				(propertyIdNumb == PidTagBody)
				)
			{
				maxDumpLength = -1;
			}
		...
		DumpTextU16(out, data, data_len, maxDumpLength);
	}
	else if (dumpMethod == 3)
	{
		...
		DumpTextU8(out, data, data_len, maxDumpLength);
	}
	else if (dumpMethod == 4)
	{
		...
		if (expandedPrint)
			maxDumpLength = -1;
		DumpRTF(out, data, data_len, maxDumpLength);
	}
	else
	{
		fprintf(out, "\n");
...
void OutlookMessage::GetToLists(std::string& To, std::string& CC, std::string& BCC)
{
	...
	for (it = m_recipList.begin(); it != m_recipList.end(); ++it)
	{
		...
		int dirType = 2;  // dirType = 2 indicates recipient or attachment object
		DWORD propertyId = PidTagRecipientType;
		UINT64 value;
		UINT16 type;
		bool valid = OutlookMessage::GetDirPropertyValueFixedLength(this->m_cfbf, dirType, entry.m_properties_version1_0, propertyId, value, type);
		if (!valid)
...
bool OutlookMessage::GetMessageCodepage(UINT64& nMessageCodepage, std::string& MessageCodepage)
{
	...
	int dirType = m_baseDepthLevel;
	if (m_baseDepthLevel == 0)
		int deb = 1;
	if (m_baseDepthLevel == 1)
		int deb = 1;
	_ASSERTE(m_baseDepthLevel <= 1);
	UINT16 type = 0;
	bool found = OutlookMessage::GetFixedLengthPropertValue(this->m_cfbf, dirType, m_body.m_properties_version1_0, propertyId, nMessageCodepage, type, MessageCodepage, false);
	return found;
}
...
bool OutlookMessage::GetInternetCodepage(UINT64& nInternetCodepage, std::string& InternetCodepage)
{
	...
	_ASSERTE(m_baseDepthLevel <= 1);
	UINT16 type = 0;
	bool found = OutlookMessage::GetFixedLengthPropertValue(this->m_cfbf, dirType, m_body.m_properties_version1_0, propertyId, nInternetCodepage, type, InternetCodepage, false);
	return found;
}
...
bool OutlookMessage::GetStoreSupportMask(UINT64& nStoreSupportMask, std::string& StoreSupportMask)
{
	...
	_ASSERTE(m_baseDepthLevel <= 1);
	UINT16 type = 0;
	bool found = OutlookMessage::GetFixedLengthPropertValue(this->m_cfbf, dirType, m_body.m_properties_version1_0, propertyId, nStoreSupportMask, type, StoreSupportMask, true);
	return found;
}
...
bool OutlookMessage::GetNativeBody(UINT64& nNativeBody, std::string& NativeBody)
{
	...
	_ASSERTE(m_baseDepthLevel <= 1);
	UINT16 type = 0;
	bool found = OutlookMessage::GetFixedLengthPropertValue(this->m_cfbf, dirType, m_body.m_properties_version1_0, propertyId, nNativeBody, type, NativeBody, false);
	return found;
}
...
bool OutlookMessage::GetDate(std::string& Date)
{
	...
	UINT64 value = 0;
	UINT16 type;
	bool found = OutlookMessage::GetDirPropertyValueFixedLength(this->m_cfbf, dirType, m_body.m_properties_version1_0, propertyId, value, type);
	if (!found)
	{
...
bool OutlookMessage::GetFixedLengthPropertValue(struct cfbf* cfbf, int dirType, DirEntry* entry, UINT16 propertyId, UINT64& nValue, UINT16& type, std::string& sValue,  bool hexFormat)
{
	sValue.clear();
	nValue = 0;

	bool found = OutlookMessage::GetDirPropertyValueFixedLength(this->m_cfbf, dirType, m_body.m_properties_version1_0, propertyId, nValue, type);
	if (found)
	{
...
bool OutlookMessage::GetDirPropertyValueFixedLength(struct cfbf* cfbf, int dirType, DirEntry* entry, UINT16 propertyId, UINT64 &value, UINT16 &type)
{
	std::string errorText;
	value = 0;
	type = 0;
...
void Attachment::Print(struct cfbf* cfbf, int level)
{
	...
	if (this->m_OutlookMessage) {
		this->m_OutlookMessage->m_baseDepthLevel = 1;
	...
	if (isMultipartSigned)
	{
		...
		if (m_attachList.size())
		{
			...
			for (it = m_attachList.begin(); it != m_attachList.end(); ++it)
			{
				...
				bool found_PidTagAttachFlags = OutlookMessage::GetFixedLengthPropertValue(this->m_cfbf, dirType, m_body.m_properties_version1_0, propertyId,
				...
				bool found_PidTagAttachEncoding = OutlookMessage::GetDirPropertyValueFixedLength(this->m_cfbf, dirType, it->m_attach.m_properties_version1_0, propertyId_PidTagAttachEncoding,
...
attachments:
	...
	if (hasValidAttachments && m_attachList.size())
	{
		...
		for (it = m_attachList.begin(); it != m_attachList.end(); ++it)
		{
			...
			bool found_PidTagAttachMethod = OutlookMessage::GetFixedLengthPropertValue(this->m_cfbf, dirType, m_body.m_properties_version1_0, propertyId,
...
BOOL ConvertOutlookMsg2Eml(CString& msgFileNamePath, CString& emlFileNamePath, CString& erText)
{
	...
	std::string msg_utf8;
	msgHelper.msg.m_baseDepthLevel = 0;
