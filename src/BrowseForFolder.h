...
public:
	...
	CBrowseForFolder(const HWND hParent, const int clsid , const int nTitleID = 0);
	...
	//
	bool SelectFolder();
	bool SetDefaultFolder(CString path);
...
private:
	...
	wchar_t m_pchTitle[MAX_PATH];

	BROWSEINFO m_bi;
	wchar_t m_szSelected[MAX_PATH];
	CString m_strPath;
	HWND m_hwnd;
...
inline bool CBrowseForFolder::SetDefaultFolder(CString path)
{
