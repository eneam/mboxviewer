            ...
            */
			...
			try
			{
				using (var inputStream = new FileInfo(inputWinmailFilePath).OpenRead())
			    {
					...
					if (htmlPartExists)
                    {
                        ; // Done. Ignore "text/rtf" even if exists
					}
				    else foreach (var bodyPart in message.BodyParts)
					{
                        bool done = false;
                        if (!bodyPart.IsAttachment)
                        {
                            ...
                            if ((part.ContentType.MediaType == "text") && (part.ContentType.MediaSubtype == "rtf"))
                            {
								...
								// Assume Body part text is automatically decoded ??
								// public static byte[] FromBase64String(string s);
								// MimeKit.Encodings.QuotedPrintableDecoder()
                                ...
                                try
                                {
                                    using (var memory = new MemoryStream())
                                    {
                                        ...
                                        part.WriteTo(memory, contentOnly);

                                        var buffer = memory.ToArray();
                                        var text = Encoding.UTF8.GetString(buffer);

                                        string res = converter.rtf2html(ref text, ref resultSB);
                                    }
                                }
								catch (Exception ex)
								{
